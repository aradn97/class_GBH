...
int perturbations_vector_init(
  ...
  if (_scalars_) {
    ...
    //TEST!!!if(ppr->gbh_fluid_approximation != gbh_fa_caio) // if we're using Caio's approximation, we don't need to integrate the shear
      ...
      if ((pa_old[ppw->index_ap_tca] == (int)tca_on) && (ppw->approx[ppw->index_ap_tca] == (int)tca_off)) {
        ...
        if (pba->has_gbh_pt == _TRUE_) {          
          if (ppw->approx[ppw->index_gbh_fa] == (int)gbh_fa_on) {
            ...
            ppv->y[ppv->index_pt_theta_gbh] =
                ppw->pv->y[ppw->pv->index_pt_theta_gbh];
            //TEST!!!if(ppr->gbh_fluid_approximation != gbh_fa_caio)
              ppv->y[ppv->index_pt_sigma_gbh] =
                ppw->pv->y[ppw->pv->index_pt_sigma_gbh];          
          }
          else {
    ...
    /*GBH_pt_start*/
      if (pba->has_gbh_pt == _TRUE_) {
        if (ppw->approx[ppw->index_gbh_fa] == (int)gbh_fa_on){ 
          ...
          ppw->pv->y[ppw->pv->index_pt_Delta_gbh] = delta_ur / 3.; /* gbh density */
          ppw->pv->y[ppw->pv->index_pt_Sigma_gbh] = theta_ur / k;
          ppw->pv->y[ppw->pv->index_pt_Sigma_gbh+1] = shear_ur;
          ppw->pv->y[ppw->pv->index_pt_Sigma_gbh+2] = l3_ur; //if you change this at some point, remember to change the for loop on ll in the next few lines
          for(n=1;n<ppw->pv->n_max_gbh;n++)
          {
            ppw->pv->y[ppw->pv->index_pt_Delta_gbh+n] =
              ppw->pv->y[ppw->pv->index_pt_Delta_gbh];
            for(ll=1;ll<4;ll++)
            {
              ppw->pv->y[ppw->pv->index_pt_Sigma_gbh+n*ppw->pv->l_max_gbh+ll-1] =
                ppw->pv->y[ppw->pv->index_pt_Sigma_gbh+ll-1];
              }
            }
        }
      }
...
int perturbations_derivs(double tau,
  ...
  if (_scalars_) {
    ...
    if (pba->has_idm == _TRUE_){
      rho_m += pvecback[pba->index_bg_rho_idm]; //GBH_pt
    ...
    /** - ---> baryon density */
    rho_m += pvecback[pba->index_bg_rho_b]; //GBH_pt
    ...
    if (pba->has_cdm == _TRUE_) {
      rho_m += pvecback[pba->index_bg_rho_cdm]; //GBH_pt
    ...
    if (pba->has_dcdm == _TRUE_) {
      rho_m += pvecback[pba->index_bg_rho_dcdm]; //GBH_pt
    ...
    if (pba->has_ur == _TRUE_) {
      rho_m += pvecback[pba->index_bg_rho_ur]; //GBH_pt
    ...
    if(pba->has_gbh_pt == _TRUE_){
      ...
      if (ppt->gauge == newtonian) {
        ...
        if (ppr->gbh_fluid_approximation == gbh_fa_caio) {//TEST!!!
          ...
          k_fs_gbh = 0.;//TEST!!!2. * _PI_ * a_prime_over_a / sqrt(3.*c2_asp);//sqrt(3. / 2. * Omega_m) * a_prime_over_a / sqrt(c2_asp);TEST!!!
          k_horizon_gbh = 0.;//TEST!!!2. * _PI_ / pvecback[pba->index_bg_horizon_gbh];
          ceff2_gbh = 0.;//TEST!!!ca2_gbh + (c2_asp - ca2_gbh) * exp(-4. / 3. * k_fs_gbh / k);
          //TEST!!!sigma_gbh=1./k2 * (-2./5. * k_horizon_gbh / k * exp(-k_horizon_gbh / k) * ceff2_gbh / (1. + w_gbh) *k2*y[pv->index_pt_delta_gbh] + k/k_fs_gbh*exp(-5.*k_fs_gbh/k)*w_gbh*w_gbh*y[pv->index_pt_theta_gbh]);
          sigma_gbh =0.;//TEST!!!

          //TEST!!! the following two lines are for test. remove them later
          ceff2_gbh = ca2_gbh;
          cvis2_gbh = 3.*w_gbh*ca2_gbh;
        }
        ...
        //TEST!!! this whole condition is for test. remove it later
        if (ppr->gbh_fluid_approximation == gbh_fa_caio) {

          dy[pv->index_pt_sigma_gbh] = -3.0*(a_prime_over_a*(2.0/3.-ca2_gbh-pseudo_p_gbh/p_gbh/3.)+1./tau)*y[pv->index_pt_sigma_gbh]+8.0/3.0*cvis2_gbh/(1.0+w_gbh)*s_l[2]*(y[pv->index_pt_theta_gbh]+metric_ufa_class);
